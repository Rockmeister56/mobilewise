...
// 🚨 NUCLEAR VOICE SYSTEM OVERRIDE
console.log('🚨 INSTALLING NUCLEAR VOICE OVERRIDE...');

// Override the main voice trigger function
const originalShowDirectSpeakNow = window.showDirectSpeakNow;
window.showDirectSpeakNow = function() {
    // 🚨 CHECK IF WE'RE IN TEXT MODE - BLOCK VOICE
    if (window.voiceModeEnabled === false) {
        console.log('🛑 VOICE BLOCKED: Text mode active - suppressing Speak Now');
        return; // COMPLETELY BLOCK VOICE
    }
    
    console.log('✅ VOICE ALLOWED: Voice mode active - proceeding normally');
    return originalShowDirectSpeakNow.apply(this, arguments);
};

// Override the banner trigger system too
const originalTriggerBannerSequence = window.triggerBannerSequence;
if (window.triggerBannerSequence) {
    window.triggerBannerSequence = function() {
        if (window.voiceModeEnabled === false) {
            console.log('🛑 BANNER SEQUENCE BLOCKED: Text mode active');
            return;
        }
        return originalTriggerBannerSequence.apply(this, arguments);
    };
}

// Enhanced toggle functions with VOICE BLOCKING
function toggleInputMode() {
    console.log('🎵 TOGGLE DANCE! Current mode:', window.voiceModeEnabled ? 'VOICE' : 'TEXT');
...
function switchToTextMode() {
    console.log('📝 SWITCHING TO TEXT MODE - NUCLEAR VOICE BLOCK');
    window.voiceModeEnabled = false;
    ...
    // Remove ALL voice elements
    const voiceElements = document.querySelectorAll('.speak-now-banner, [class*="speakNow"], .voice-overlay, .listening-overlay');
    voiceElements.forEach(el => {
        console.log('💥 Removing voice element:', el.className);
        el.remove();
    });
    ...
    console.log('✅ TEXT MODE ACTIVATED - VOICE SYSTEM BLOCKED');
}
...
function switchToVoiceMode() {
    ...
    // Allow voice system to work again
    setTimeout(() => {
        console.log('🎤 VOICE MODE ACTIVATED - allowing voice system');
        if (originalShowDirectSpeakNow) {
            originalShowDirectSpeakNow();
        }
    }, 1000);
}

console.log('✅ NUCLEAR VOICE OVERRIDE INSTALLED');
