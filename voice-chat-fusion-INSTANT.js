...
// ===========================================
// 📱 MOBILE VOICE DEBUGGER - Save logs to localStorage
// ===========================================
window.mobileVoiceDebug = [];

// Override mobileWiseVoice.speak to log EVERY call
...
window.mobileWiseVoice.speak = async function(text, options = {}) {
    ...
    const logEntry = {
        ...
        elevenLabsEnabled: VOICE_CONFIG.elevenlabs.enabled,
        textLength: text.length,
        textPreview: text.substring(0, 50)
    };
    
    window.mobileVoiceDebug.push(logEntry);
    
    // Save to localStorage (for checking on desktop)
    try {
        localStorage.setItem('mobileVoiceDebug', JSON.stringify(window.mobileVoiceDebug));
    } catch (e) {
        console.log("Could not save to localStorage:", e);
    }
    
    console.log(`📱 SPEAK CALL #${window.mobileVoiceDebug.length}:`);
    console.log(`   Mobile: ${isMobile}, Provider: ${VOICE_CONFIG.provider}`);
    console.log(`   Text: "${logEntry.textPreview}..."`);
    
    // Force ElevenLabs on mobile
    if (isMobile) {
        console.log("📱 FORCING ELEVENLABS FOR MOBILE");
...
// Function to check logs (run on desktop after mobile test)
window.checkMobileVoiceLogs = function() {
    const logs = JSON.parse(localStorage.getItem('mobileVoiceDebug') || '[]');
    console.log("📊 === MOBILE VOICE LOGS ===");
    console.log(`Total calls: ${logs.length}`);
    
    logs.forEach((log, i) => {
        console.log(`\n#${i + 1} - ${log.timestamp}`);
        console.log(`   Mobile: ${log.isMobile}`);
        console.log(`   Provider: ${log.provider}`);
        console.log(`   ElevenLabs enabled: ${log.elevenLabsEnabled}`);
        console.log(`   Text: "${log.textPreview}..."`);
        
        // Check if provider changed
        if (i > 0 && logs[i-1].provider !== log.provider) {
            console.log(`   🚨 PROVIDER CHANGED: ${logs[i-1].provider} → ${log.provider}`);
        }
    });
    
    if (logs.length === 0) {
        console.log("No logs found. Make sure to test on mobile first!");
    }
};

console.log("✅ Mobile voice debugger installed");
