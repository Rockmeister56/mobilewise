...
// =============================================================================
// 🎯 MOBILEWISE AI - COMPLETE CONVERSATION ENGINE WITH TRIGGER INTEGRATION
// =============================================================================

// ===========================================
// 🎯 1. EMERGENCY DETECTION (KEEP THIS!)
// ===========================================
...
window.getAIResponse = async function(userMessage, conversationHistory = []) {
    ...
    console.log('🎯 getAIResponse called:', userMessage.substring(0, 50) + '...');
    
    // 🎯 EMERGENCY DETECTION (YOUR EXISTING CODE)
    if ((lowerMessage.includes('yes') || lowerMessage.includes('yeah') || lowerMessage.includes('sure')) &&
        ...
        console.log('🎯 EMERGENCY YES DETECTED - Triggering Action Center IMMEDIATELY');
    ...
    // 🎯 NEW: MOBILEWISE AI PROCESSING
    const mobilewiseResponse = await MobileWiseAI.getAIResponse(userMessage);
    return mobilewiseResponse;
};

// ===========================================
// 🎯 2. MOBILEWISE AI ENGINE
// ===========================================
const MobileWiseAI = {
    // 🎯 CONVERSATION STATES
    state: 'introduction',
    ...
    aboutUs: {
        ...
        value: "We're not another chatbot - we're your unfair advantage, with 25 years of weapons-grade marketing IQ built in."
    },
    ...
    async getAIResponse(userMessage) {
        console.log(`🤖 MobileWise AI processing: "${userMessage.substring(0, 50)}..."`);
        
        const lowerMsg = userMessage.toLowerCase();
        
        // 🚨 URGENT/APPOINTMENT DETECTION
        if (this.detectUrgentIntent(lowerMsg)) {
            return this.handleUrgentRequest();
        }
        
        // 🚨 CONCERN DETECTION
        if (this.detectConcern(lowerMsg)) {
            return this.handleConcern(userMessage);
        }
        
        // 🎯 CATEGORY 1: ABOUT MOBILEWISE AI QUESTIONS
        if (this.isAboutUsQuestion(lowerMsg)) {
            return this.handleAboutUsQuestion(lowerMsg);
        }
        
        // 🎯 CATEGORY 2: ABOUT AI BENEFITS QUESTIONS
        if (this.isAIBenefitsQuestion(lowerMsg)) {
            return this.handleAIBenefitsQuestion(lowerMsg);
        }
        
        // 🎯 CATEGORY 3: GENERAL BUSINESS/QUALIFICATION QUESTIONS
        return this.handleBusinessConversation(userMessage);
    },
    
    // ===========================================
    // 🎯 CATEGORY 1: ABOUT MOBILEWISE AI
    // ===========================================
    isAboutUsQuestion(message) {
        const triggers = [
    ...
    handleAboutUsQuestion(message) {
        if (message.includes('how long') || message.includes('experience') || message.includes('been in business')) {
            return `${this.aboutUs.founder}. We've been deploying AI solutions that actually close deals for 3 years, but our marketing expertise spans 25 years. ${this.aboutUs.value}`;
        }
        
        if (message.includes('brett') || message.includes('founder') || message.includes('owner')) {
            return `Our founder is ${this.aboutUs.founder}. His philosophy: "good enough is the enemy of holy shit." He built MobileWise AI to be the unfair advantage he wished he had 25 years ago.`;
        }
        
        if (message.includes('proof') || message.includes('results') || message.includes('case study')) {
            const randomProof = this.aboutUs.proof[Math.floor(Math.random() * this.aboutUs.proof.length)];
        ...
        return `MobileWise AI was founded by ${this.aboutUs.founder}. We specialize in AI that doesn't just chat - it closes deals. ${this.aboutUs.value} Would you like to hear some specific results we've achieved?`;
    },
    
    // ===========================================
    // 🎯 CATEGORY 2: AI BENEFITS QUESTIONS
    // ===========================================
    isAIBenefitsQuestion(message) {
        const triggers = [
    ...
    handleAIBenefitsQuestion(message) {
        // Industry-specific benefits
        if (this.user.businessType) {
            const benefit = this.industryBenefits[this.user.businessType];
            if (benefit) {
                return `For ${this.user.businessType} businesses like yours: ${benefit}. Plus: ${this.getRandomStat()}`;
            }
        }
        
        // ROI/Value questions
        if (message.includes('roi') || message.includes('worth') || message.includes('cost') || message.includes('price')) {
            return `Our AI typically pays for itself within 30-90 days. Statistically: ${this.getRandomStat()}. Most clients see 3-5x ROI within 6 months. Would you like our pricing guide?`;
        }
    ...
    getRandomStat() {
        return this.aboutUs.stats[Math.floor(Math.random() * this.aboutUs.stats.length)];
    },
    
    // ===========================================
    // 🎯 CATEGORY 3: BUSINESS CONVERSATION FLOW
    // ===========================================
    handleBusinessConversation(userMessage) {
        const lowerMsg = userMessage.toLowerCase();
        
        switch(this.state) {
            case 'introduction':
                return this.handleIntroduction(lowerMsg);
                
            case 'qualification':
                return this.handleQualification(lowerMsg);
                
            case 'needs_assessment':
                return this.handleNeedsAssessment(lowerMsg);
                
            case 'solution_presentation':
                return this.handleSolutionPresentation(lowerMsg);
                
            case 'closing':
                return this.handleClosing(lowerMsg);
    ...
    handleIntroduction(message) {
        ...
        if (name) {
            ...
            return `Nice to meet you, ${name}! What type of business do you have? (Retail, service, healthcare, legal, etc.)`;
        }
        
        return "Hi! I'm Sophia, your MobileWise AI assistant. We build AI that actually closes deals. What's your name?";
    },
    
    handleQualification(message) {
        ...
        for (const [keyword, type] of Object.entries(businessTypes)) {
            if (message.includes(keyword)) {
                ...
                this.state = 'needs_assessment';
                return `Great! ${type} businesses are perfect for AI. What's your #1 challenge right now? (Leads, conversions, customer service, etc.)`;
            }
        }
        
        return "Thanks! What industry are you in? This helps me share the most relevant AI benefits for your business.";
    },
    ...
    handleNeedsAssessment(message) {
        ...
        if (message.includes('lead') || message.includes('customer') || message.includes('client')) {
            this.user.challenge = 'leads';
            this.state = 'solution_presentation';
            return "Getting enough qualified leads? Our AI increases lead capture by 40-60% typically. It engages visitors 24/7 and qualifies them instantly. Would you like to know how we do it?";
        }
        
        if (message.includes('convert') || message.includes('sale') || message.includes('close')) {
            this.user.challenge = 'conversions';
            this.state = 'solution_presentation';
            return "Conversion leaks are profit killers! Our AI guides prospects through objections and closes deals automatically. Typical results: 23%+ conversion increase. Want to see how?";
        }
        
        if (message.includes('service') || message.includes('support') || message.includes('answer')) {
            this.user.challenge = 'service';
            this.state = 'solution_presentation';
            return "Customer service eating your time? Our AI handles 70%+ of inquiries instantly, 18% faster than humans. Frees your team for high-value work. Interested in the details?";
        }
        
        this.state = 'solution_presentation';
        return "Thanks for sharing! Based on what you've told me, here's how AI could help...";
    },
    
    handleSolutionPresentation(message) {
        const positiveResponses = ['yes', 'yeah', 'sure', 'ok', 'tell me', 'how', 'show me'];
        
        if (positiveResponses.some(r => message.includes(r))) {
            this.user.interestLevel = 'high';
            this.state = 'closing';
            
            // 🎯 SET INTENT CONTEXT FOR TRIGGER DETECTION
            let offerText = '';
            
            if (this.user.interestLevel === 'high') {
                offerText = `Perfect! I recommend our ${this.offers.tier3}. We'll analyze your current conversion leaks and show exactly where AI could boost your revenue. Would Tuesday at 3pm work for your free analysis?`;
                window.lastPreCloseIntent = 'mobilewise_analysis';
            } else {
                offerText = `Great! Would you prefer: 1) ${this.offers.tier1}, 2) ${this.offers.tier2}, or 3) ${this.offers.tier4}?`;
    ...
    detectUrgentIntent(message) {
        const urgentPatterns = ['urgent', 'now', 'immediately', 'asap', 'emergency', 'call me'];
        return urgentPatterns.some(pattern => message.includes(pattern));
    },
    
    detectConcern(message) {
        const concernPatterns = ['expensive', 'cost', 'price', 'trust', 'believe', 'time', 'busy'];
        return concernPatterns.some(pattern => message.includes(pattern));
    },
    ...
    handleUrgentRequest() {
        ...
        return "I understand this is urgent! I've opened our immediate connection options right now.";
    },
    
    handleConcern(message) {
        if (window.handleConcernWithTestimonial) {
            window.handleConcernWithTestimonial(message, 'general');
        }
        return "I understand your concern. Let me show you what other business owners experienced when they had similar questions...";
    }
};
...
// ===========================================
const enhancedGetAIResponse = window.getAIResponse;
window.getAIResponse = async function(userMessage, conversationHistory = []) {
    const lowerMessage = userMessage.toLowerCase();
    
    // 🎯 ENHANCED YES DETECTION FOR ALL OFFERS
    const yesPatterns = ['yes', 'yeah', 'sure', 'okay', 'ok', 'absolutely', 'definitely', 'let\'s do it', 'ready', 'go ahead'];
    
    if (yesPatterns.some(pattern => lowerMessage.includes(pattern)) && window.lastPreCloseIntent) {
        console.log('🎯 ENHANCED TRIGGER DETECTED - User said YES to:', window.lastPreCloseIntent);
        ...
        setTimeout(() => {
            if (window.triggerLeadActionCenter) {
                window.triggerLeadActionCenter();
                console.log('✅ Action Center triggered for intent:', triggeredIntent);
            }
        }, 300);
        
        return "Perfect! I've opened our communication relay center. Click any button that works best for you!";
    }
    
    // Process through enhanced function
    return await enhancedGetAIResponse.apply(this, arguments);
};

console.log('✅ COMPLETE MobileWise AI Conversation Engine Loaded with Trigger Integration!');
