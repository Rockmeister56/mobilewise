...
// ================================================
// 🎯 MOBILE VOICE DOUBLE-FIX - ADD THIS RIGHT HERE!
// ================================================
(function() {
    // Wait for page to load
    setTimeout(() => {
        if (isMobileDevice()) {
            console.log("📱 MOBILE DETECTED: Installing voice double-protection");
            
            // Mobile speech state tracker
            window.mobileVoiceLock = {
                isSpeaking: false,
                lastSpeechStart: 0,
                speechQueue: [],
                audioElements: []
            ...
            // 1. PROTECT MAIN SPEAK FUNCTIONS
            const protectSpeakFunction = (funcName) => {
                if (window[funcName] && typeof window[funcName] === 'function') {
                    const originalFunc = window[funcName];
                    window[funcName] = function(text) {
                        const now = Date.now();
                        const lock = window.mobileVoiceLock;
                        
                        // If speech happened within 1 second, block it
                        if (lock.isSpeaking || (now - lock.lastSpeechStart < 1000)) {
                            console.log(`📱 Mobile: Voice busy, skipping ${funcName} call`);
                            console.log(`   Last speech: ${now - lock.lastSpeechStart}ms ago`);
                            console.log(`   Text: "${text?.substring?.(0, 50)}..."`);
                            return Promise.resolve();
                        }
                        
                        lock.isSpeaking = true;
                        lock.lastSpeechStart = now;
                        console.log(`📱 Mobile: ${funcName} started at ${now}`);
                        
                        const result = originalFunc.call(this, text);
                        
                        // Auto-release lock after expected speech duration
                        const estimatedDuration = (text?.length || 0) * 70; // ~70ms per character
                        setTimeout(() => {
                            lock.isSpeaking = false;
                            console.log(`📱 Mobile: Voice lock released after ${estimatedDuration}ms`);
                        }, Math.max(estimatedDuration, 2000));
                        
                        return result;
                    };
                    console.log(`✅ Protected ${funcName} for mobile`);
                }
            };
            
            // Protect all possible speak functions
            ['speakText', 'speak', 'speakResponse', 'addAIMessage'].forEach(protectSpeakFunction);
            
            // 2. PROTECT AUDIO ELEMENTS
            const originalCreateElement = document.createElement;
            document.createElement = function(tagName) {
                const element = originalCreateElement.call(this, tagName);
                
                if (tagName.toLowerCase() === 'audio') {
                    console.log("📱 Mobile: Audio element created");
                    window.mobileVoiceLock.audioElements.push(element);
                    ...
                    element.play = function() {
                        ...
                        if (lock.isSpeaking) {
                            console.log("🚫 Mobile: Audio play blocked - already speaking");
                            return Promise.reject(new Error('Mobile voice lock active'));
                        }
                        
                        lock.isSpeaking = true;
                        console.log("📱 Mobile: Audio play allowed");
                        ...
                        this.addEventListener('ended', () => {
                            setTimeout(() => {
                                lock.isSpeaking = false;
                                console.log("📱 Mobile: Audio lock released (ended)");
                            }, 300);
                        }, { once: true });
            ...
            console.log("✅ Mobile voice double-protection installed");
            
            // 3. MONITOR FOR DOUBLE EVENTS (mobile-specific)
            let lastClickTime = 0;
            document.addEventListener('click', (e) => {
                const now = Date.now();
                if (now - lastClickTime < 300) {
                    console.log("⚠️ Mobile: Rapid click detected!", now - lastClickTime, "ms apart");
                }
                lastClickTime = now;
            }, true);
            ...
            document.addEventListener('touchend', (e) => {
                const now = Date.now();
                if (now - lastClickTime < 300) {
                    console.log("⚠️ Mobile: Rapid touch detected!", now - lastClickTime, "ms apart");
                }
                lastClickTime = now;
            }, true);
        }
