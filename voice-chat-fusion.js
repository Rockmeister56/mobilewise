...
// AI Avatar Controller - Mobile-Wise AI Formviser
class AIAvatarController {
    constructor() {
        // Get all required elements
        this.staticState = document.getElementById('avatarContainer');
        this.videoState = document.getElementById('videoState');
        ...
        if (!this.videoState) {
            console.error('❌ videoState element not found!');
            return;
        }
        if (!this.avatar) {
            console.error('❌ aiAvatar element not found!');
            return;
        }
        if (!this.micButton) {
        ...
        console.log('✅ All avatar elements found - initializing...');
        this.init();
    }
    ...
    init() {
        ...
        this.micButton.addEventListener('click', () => {
            console.log('🎤 Microphone button clicked - activating avatar...');
            this.activateAvatar();
        });
        
        // Video ended - transition to interactive mode
        this.avatar.addEventListener('ended', () => {
            console.log('🎬 Avatar video ended - starting voice chat...');
            this.startVoiceChat();
        });
    ...
    activateAvatar() {
        // Hide static state with fade effect
        this.staticState.classList.add('fade-out');
        
        setTimeout(() => {
            // Hide static container
            this.staticState.style.display = 'none';
            
            // Show video container
            this.videoState.style.display = 'block';
            this.videoState.classList.add('fade-in');
            
            // Play introduction video
            this.avatar.play().catch(error => {
                console.error('❌ Failed to play avatar video:', error);
                // Fallback to voice chat if video fails
                this.startVoiceChat();
            });
            
            console.log('🎭 Avatar activated - video should be playing...');
        }, 500);
    }
    
    startVoiceChat() {
        console.log('🗣️ Starting voice chat mode...');
        
        // Fade out video after brief delay
        setTimeout(() => {
            this.videoState.classList.add('fade-out');
            
            setTimeout(() => {
                this.videoState.style.display = 'none';
    ...
    showVoiceInterface() {
        ...
        voiceInterface.className = 'voice-interface fade-in';
        voiceInterface.innerHTML = `
            <div class="voice-controls">
                <div class="listening-indicator">
                    <div class="pulse-animation"></div>
                    <h3>I'm ready to help!</h3>
                    <p>Speak naturally and I'll assist with your form...</p>
                </div>
                <div class="voice-actions">
                    <button id="muteBtn" class="voice-btn">🔇 Mute</button>
                    <button id="restartBtn" class="voice-btn">🔄 Restart</button>
                </div>
            </div>
        `;
        ...
        console.log('✅ Voice interface created and ready!');
    }
    
    initVoiceRecognition() {
        // Placeholder for voice service integration
        console.log('🎯 Voice recognition ready - integrate your speech services here!');
        ...
        if (restartBtn) {
            restartBtn.addEventListener('click', () => {
                console.log('🔄 Restarting avatar experience...');
                this.restart();
            });
        }
    }
    ...
    restart() {
        ...
        // Reset all states
        this.staticState.style.display = 'block';
        this.staticState.classList.remove('fade-out');
        this.videoState.style.display = 'none';
        this.videoState.classList.remove('fade-in', 'fade-out');
        
        console.log('🔄 Avatar experience restarted!');
    }
}

// Initialize when page loads - EMPIRE MODE ACTIVATED! 🚀
document.addEventListener('DOMContentLoaded', () => {
    console.log('🚀 Mobile-Wise AI Formviser - Avatar System Loading...');
    const aiController = new AIAvatarController();
    
    // Make it globally accessible for debugging
    window.avatarController = aiController;
    
    console.log('✅ Avatar Controller loaded - Ready for CONVERSION DOMINATION! 💎');
});
