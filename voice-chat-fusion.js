...
// ========================================
// 📱 PRODUCTION MOBILE BEEP ELIMINATION
// For Real Mobile Devices (iOS Safari, Android Chrome)
// ADD THIS TO YOUR WEBSITE'S MAIN JS FILE
// ========================================

(function() {
    'use strict';
    
    console.log('📱 PRODUCTION: Mobile beep elimination loading for real devices...');
    ...
    if (!isRealMobileDevice()) {
        console.log('📱 Not a mobile device - beep fix not needed');
        return;
    }
    
    console.log('📱 MOBILE DEVICE DETECTED - Applying beep elimination...');
    
    // ===========================================
    // MOBILE SYSTEM BEEP SUPPRESSION
    // ===========================================
    
    // Method 1: Override Audio constructor for mobile system sounds
    (function suppressSystemAudio() {
        const OriginalAudio = window.Audio;
        
        window.Audio = function(src) {
            const audio = new OriginalAudio(src);
            
            // Detect mobile system sounds (typically short data URLs or blobs)
            if (src && typeof src === 'string') {
                ...
                if (isSystemSound) {
                    console.log('🔇 MOBILE: Suppressed system audio beep');
                    audio.volume = 0;
                    audio.muted = true;
                    ...
                    audio.play = function() {
                        console.log('🔇 MOBILE: Blocked system audio play');
        ...
        console.log('✅ MOBILE: Audio constructor override active');
    })();
    
    // Method 2: Suppress Web Audio API system sounds
    (function suppressWebAudio() {
        if (typeof AudioContext !== 'undefined' || typeof webkitAudioContext !== 'undefined') {
            const OriginalAudioContext = window.AudioContext || window.webkitAudioContext;
            
            const createSuppressedContext = function(...args) {
                const context = new OriginalAudioContext(...args);
                
                // Override methods that might create system sounds
                const originalCreateOscillator = context.createOscillator;
                context.createOscillator = function() {
                    const oscillator = originalCreateOscillator.call(this);
                    
                    // Suppress short frequency oscillations (common in system beeps)
                    const originalStart = oscillator.start;
                    oscillator.start = function(when, offset, duration) {
                        if (duration && duration < 0.5) { // Less than 500ms - likely a beep
                            console.log('🔇 MOBILE: Suppressed oscillator beep');
                            return;
                        }
            ...
            if (window.AudioContext) window.AudioContext = createSuppressedContext;
            if (window.webkitAudioContext) window.webkitAudioContext = createSuppressedContext;
            
            console.log('✅ MOBILE: Web Audio API override active');
        }
    })();
    
    // ===========================================
    // SPEECH RECOGNITION BEEP SUPPRESSION
    // ===========================================
    
    function suppressSpeechRecognitionBeep() {
        // Wait for speech recognition to be initialized
        const checkForRecognition = setInterval(() => {
            if (window.recognition || window.webkitSpeechRecognition || window.SpeechRecognition) {
                clearInterval(checkForRecognition);
                ...
                if (recognition) {
                    ...
                    recognition.start = function() {
                        console.log('🎤 MOBILE: Starting speech recognition with beep suppression');
                        
                        // Pre-suppress any pending audio
                        try {
                            // Mute system volume briefly (iOS Safari specific)
                            if (/iphone|ipad|ipod/i.test(navigator.userAgent)) {
                                const silentAudio = new Audio('data:audio/mp3;base64,SUQzBAAAAAABEVRYWFgAAAAtAAADY29tbWVudABCaWdTb3VuZEJhbmsuY29tIC8gTGFTb25vdGhlcXVlLm9yZwBURU5DAAAAHQAAATEI');
                            ...
                            if (/android/i.test(navigator.userAgent)) {
                                // Create audio context to take focus from system sounds
                            ...
                            console.log('🔇 MOBILE: Audio suppression method not available');
                        }
                        
                        // Start actual recognition after brief delay for suppression to take effect
                        setTimeout(() => {
                            try {
                    ...
                    // Also suppress the end beep
                    const originalStop = recognition.stop;
                    recognition.stop = function() {
                        console.log('🎤 MOBILE: Stopping speech recognition (beep suppressed)');
                        
                        // Suppress end beep
                        setTimeout(() => {
                            document.querySelectorAll('audio').forEach(audio => {
                                if (!audio.dataset.customSound && audio.duration < 1) {
                                    audio.pause();
                                    audio.volume = 0;
                                }
                            });
                    ...
                    console.log('✅ MOBILE: Speech recognition beep suppression active');
                }
            }
        }, 100);
        
        // Stop checking after 10 seconds
        setTimeout(() => clearInterval(checkForRecognition), 10000);
    }
    
    // Apply speech recognition suppression
    suppressSpeechRecognitionBeep();
    
    // ===========================================
    // TIMING SYNCHRONIZATION FOR MOBILE
    // ===========================================
    
    // Override common timing functions to eliminate delays
    function optimizeMobileTiming() {
        // Reduce mobile-specific delays
        if (window.speechRecognitionConfig) {
            window.speechRecognitionConfig.mobileDelay = 50; // Reduced from higher values
            window.speechRecognitionConfig.startupDelay = 25;
            console.log('⚡ MOBILE: Timing delays optimized');
        }
        
        // Override setTimeout for speech-related functions to reduce delays
        const originalSetTimeout = window.setTimeout;
        window.setTimeout = function(callback, delay, ...args) {
            // Reduce delays for speech recognition functions
            if (typeof callback === 'function' && 
                (callback.toString().includes('speech') || 
                 callback.toString().includes('recognition') ||
                 callback.toString().includes('listening'))) {
                
                delay = Math.min(delay, 100); // Cap at 100ms for speech functions
                console.log(`⚡ MOBILE: Reduced speech timing delay to ${delay}ms`);
            }
    ...
    optimizeMobileTiming();
    
    // ===========================================
    // MOBILE-SPECIFIC CSS SUPPRESSION
    // ===========================================
    
    function appleMobileCSSSuppression() {
        const style = document.createElement('style');
        style.id = 'mobile-beep-suppression';
        style.textContent = `
            /* Mobile-specific audio element suppression */
            @media only screen and (max-width: 768px) {
                audio:not([data-custom-sound]):not([data-user-audio]) {
                ...
                /* iOS Safari specific */
                audio[src*="data:audio"]:not([data-custom-sound]) {
                    display: none !important;
                }
        ...
        document.head.appendChild(style);
        console.log('✅ MOBILE: CSS audio suppression applied');
    }
    
    // Apply CSS suppression when DOM is ready
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', appleMobileCSSSuppression);
    } else {
        appleMobileCSSSuppression();
    }
    
    // ===========================================
    // EMERGENCY MOBILE BEEP KILLER
    // ===========================================
    
    window.emergencyMobileBeepKiller = function() {
        console.log('🚨 MOBILE: Emergency beep elimination activated');
        
        // Stop all short audio clips
        document.querySelectorAll('audio').forEach(audio => {
            if (!audio.dataset.customSound && (!audio.duration || audio.duration < 2)) {
                audio.pause();
                audio.currentTime = 0;
                audio.volume = 0;
                audio.muted = true;
        ...
        // Suspend and resume audio context to clear pending sounds
        if (window.audioContext || window.AudioContext || window.webkitAudioContext) {
            try {
                const context = window.audioContext || 
                               new (window.AudioContext || window.webkitAudioContext)();
                context.suspend().then(() => {
                    setTimeout(() => context.resume(), 100);
                });
            } catch (e) {}
        }
        
        console.log('🔇 MOBILE: All potential beeps eliminated');
    };
    
    // Auto-run emergency beep killer every 30 seconds as failsafe
    setInterval(() => {
        if (document.querySelectorAll('audio:not([data-custom-sound])').length > 0) {
            window.emergencyMobileBeepKiller();
        }
    }, 30000);
    ...
    console.log('🚀 MOBILE BEEP ELIMINATION ACTIVE FOR REAL DEVICES:');
    console.log('  📱 Device Type: Real Mobile Device');
    console.log('  🔇 System Audio: SUPPRESSED');
    console.log('  🎤 Speech Recognition: BEEP-FREE');
    console.log('  ⚡ Timing: OPTIMIZED');
    console.log('  🚨 Emergency Killer: AVAILABLE');
    
    // Mark as loaded for other scripts
    window.mobileBeepElimination = {
        loaded: true,
        version: '1.0',
        deviceType: 'mobile'
    };
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = window.mobileBeepElimination;
}