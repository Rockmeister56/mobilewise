...
// ============================================
// 🎯 IMPROVED TESTIMONIAL VIEWER WITH DELETE
// ============================================

console.log('🎯 Updating All Testimonials viewer with proper design and delete buttons...');

// 1. UPDATE THE POPULATE FUNCTION WITH PROPER HEADER
function populateAllTestimonialsViewer() {
    console.log('🔄 Populating All Testimonials viewer...');
    ...
    console.log(`📊 Found ${allTestimonials.length} testimonials to display`);
    
    // Update the count badge
    const countBadge = document.getElementById('totalTestimonialsCount');
    if (countBadge) {
        countBadge.textContent = allTestimonials.length;
    }
        ...
        container.innerHTML = `
            <div class="no-testimonials-message">
                <div class="empty-state">
                    ...
                    <h3>No Videos Yet</h3>
                    <p>Add your first video testimonial or informational video.</p>
                </div>
            </div>
        `;
        return;
    ...
    let html = `
        <div class="testimonials-header">
            <div class="viewer-stats">
                <span class="stat"><strong>${allTestimonials.length}</strong> Total Videos</span>
                <span class="stat">${Object.keys(window.testimonialData.testimonialGroups || {}).length} Client Groups</span>
                <span class="stat">${Object.keys(window.testimonialData.informationalGroups || {}).length} Info Groups</span>
            </div>
            <div class="viewer-filters">
                <button class="filter-btn active" onclick="filterViewer('all')">All Videos</button>
                <button class="filter-btn" onclick="filterViewer('testimonial')">Client Testimonials</button>
                <button class="filter-btn" onclick="filterViewer('informational')">Info & How-To</button>
            </div>
        </div>
        <div class="testimonials-grid">
    `;
    
    // Add each testimonial card
    allTestimonials.forEach((t, index) => {
        const isInfo = t.type === 'informational';
        const shortText = t.text ? (t.text.length > 80 ? t.text.substring(0, 80) + '...' : t.text) : '';
        const shortTitle = t.title.length > 40 ? t.title.substring(0, 40) + '...' : t.title;
        
        // Determine icon based on content
        let icon = '⭐';
        if (isInfo) icon = '📚';
        if (t.concernType === 'price') icon = '💰';
        if (t.concernType === 'time') icon = '⏰';
        if (t.concernType === 'trust') icon = '🤝';
        if (t.concernType === 'results') icon = '📈';
        
        html += `
            <div class="testimonial-card ${isInfo ? 'info-card' : 'client-card'}" 
                 data-id="${t.id}" 
                ...
                <div class="card-content">
                    ${shortText ? `<p class="testimonial-text">"${shortText}"</p>` : ''}
                    <div class="card-meta">
                        <span class="concern-badge">${t.concernType || 'General'}</span>
                        <span class="views-count">👁️ ${t.views || 0}</span>
                        <span class="date-added">${new Date(t.addedAt).toLocaleDateString()}</span>
                    </div>
                </div>
                <div class="card-actions">
                    <button class="btn-play" onclick="playTestimonialVideo('${t.id}')" title="Play Video">
                        ▶ Play
                    </button>
                    <button class="btn-details" onclick="showTestimonialDetails('${t.id}')" title="View Details">
                        ℹ️ Details
                    </button>
                    <button class="btn-delete" onclick="deleteTestimonialFromViewer('${t.id}', '${t.groupId}', '${t.type}')" title="Delete Video">
                        🗑️ Delete
                    </button>
                </div>
            </div>
    ...
    console.log(`✅ Populated viewer with ${allTestimonials.length} testimonials`);
}

// 2. ADD DELETE FUNCTIONALITY
function deleteTestimonialFromViewer(testimonialId, groupId, type) {
    console.log(`🗑️ Attempting to delete: ${testimonialId} from group ${groupId} (${type})`);
    ...
    try {
        ...
        const array = group[arrayName];
        if (!array) {
            console.error('❌ Testimonials/videos array not found');
            alert('Error: Video array not found');
            return;
        }
        
        const index = array.findIndex(t => t.id === testimonialId);
        if (index === -1) {
            console.error('❌ Testimonial not found in array:', testimonialId);
        ...
        console.log(`✅ Removed testimonial at index ${index}`);
        
        // Update statistics
        if (window.testimonialData.statistics) {
            if (type === 'testimonial') {
                window.testimonialData.statistics.totalTestimonials = 
                    Math.max(0, (window.testimonialData.statistics.totalTestimonials || 0) - 1);
            } else {
                window.testimonialData.statistics.totalInformationalVideos = 
                    Math.max(0, (window.testimonialData.statistics.totalInformationalVideos || 0) - 1);
            }
        }
        ...
        setTimeout(() => {
            ...
            if (card) {
                ...
                setTimeout(() => {
                    ...
                    // Refresh the viewer to update counts
                    populateAllTestimonialsViewer();
                }, 300);
            } else {
            ...
            if (typeof showSuccess === 'function') {
                ...
                alert('✅ Video deleted! Click "Save Data" to update the data file.');
            }
        }, 100);
...
// 3. UPDATE STYLES FOR DELETE BUTTONS
function addViewerStyles() {
    if (document.getElementById('all-testimonials-styles')) return;
    
    const style = document.createElement('style');
    style.id = 'all-testimonials-styles';
    style.textContent = `
        ...
        .viewer-filters .filter-btn:hover {
            background: #e0e0e0;
        }
        
        .viewer-filters .filter-btn.active {
            background: #4a90e2;
            color: white;
            font-weight: 500;
        }
        ...
        .btn-play, .btn-details, .btn-delete {
            padding: 7px 12px;
            border: none;
            border-radius: 5px;
            cursor: pointer;
        ...
        .btn-play:hover {
            background: linear-gradient(135deg, #357ae8 0%, #2a65cc 100%);
    ...
    console.log('✅ Added enhanced viewer styles');
}

// 4. UPDATE THE MODAL HEADER IF POSSIBLE
function updateModalHeader() {
    // Try to update the modal title to match your design
    const modalTitle = document.querySelector('#allTestimonialsModal .modal-title');
    if (modalTitle) {
        // Check current content and update if needed
        const currentText = modalTitle.textContent || '';
        if (!currentText.includes('🎬')) {
            // Update with your preferred design
            modalTitle.innerHTML = `
                <span class="modal-icon">👁️</span>
                Video Library
                <span id="totalTestimonialsCount" class="badge">0</span>
            `;
...
// 5. ENHANCE THE MODAL SHOW FUNCTION
function enhanceModalFunctions() {
    // Update the header when modal opens
    ...
    if (modal) {
        ...
        console.log('✅ Set up modal observer');
    }
}
...
// Update existing populate function
if (typeof populateAllTestimonialsViewer === 'function') {
    console.log('✅ Replacing existing populate function');
}

// Run enhancements
enhanceModalFunctions();

// Update if modal is already open
setTimeout(() => {
    const modal = document.getElementById('allTestimonialsModal');
    if (modal && modal.style.display === 'block') {
        console.log('📊 Modal is open, updating...');
        updateModalHeader();
        populateAllTestimonialsViewer();
    }
}, 300);

console.log('✅ Enhanced viewer ready with delete buttons!');
console.log('\n🎯 New features:');
console.log('1. Proper header design');
console.log('2. Delete button on each card');
console.log('3. Better styling and animations');
console.log('4. Confirmation before delete');
...
// ============================================
// 🛠️ REPLACE SAVE ALL DATA FUNCTION
// ============================================

console.log('🔧 Replacing saveAllData() function...');

// Store the original for localStorage saving
const originalSaveToLocalStorage = window.saveToLocalStorage;

// Replace saveAllData
window.saveAllData = function() {
    console.log('💾 saveAllData() called - saving to localStorage AND downloading file');
    
    // 1. First save to localStorage (original behavior)
    ...
    // 2. Also download as JS file (NEW!)
    downloadTestimonialDataFile();
    
    // 3. Show success message
    if (typeof showSuccess === 'function') {
        showSuccess('✅ All data saved successfully! File downloaded as testimonials-data.js');
    } else {
        alert('✅ All data saved! File downloaded as testimonials-data.js');
    }
...
function downloadTestimonialDataFile() {
    ...
    const cleanData = {
        ...
        // Remove old .groups structure if it exists
        groups: undefined,
        
        // Update metadata
        __version: dataToSave.__version || "3.0-fixed",
        __generated: new Date().toISOString(),
        __notes: "Saved from management system"
    };
    
    // Remove the groups property
    delete cleanData.groups;
    
    // Create the JavaScript file content
    const jsContent = `// ===================================================
// 🎬 TESTIMONIALS DATA FILE
// Generated by Testimonial Management System
// Saved: ${new Date().toLocaleString()}
// DO NOT EDIT MANUALLY - Use the management system
// ===================================================
...
console.log('✅ saveAllData() function replaced!');
console.log('Now when you click "Save Data", it will:');
console.log('1. ✅ Save to localStorage (existing behavior)');
console.log('2. ✅ Download testimonials-data.js file (NEW!)');
