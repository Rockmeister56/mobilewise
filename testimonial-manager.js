...
// ============================================
// 🚨 DIRECT FIX FOR MISSING MODAL ELEMENTS
// ============================================
...
(function() {
    ...
    console.log('🔧 Loading direct modal element fix...');
    
    // OVERRIDE THE initGroupCreationModal FUNCTION
    const originalInitGroupCreationModal = window.initGroupCreationModal;
    
    window.initGroupCreationModal = function() {
        console.log('🎯 INIT GROUP CREATION MODAL (DIRECT FIX)');
        
        // Don't use the broken original - use our fixed version
        return fixedInitGroupCreationModal();
    };
    
    function fixedInitGroupCreationModal() {
        console.log('🔍 Fixed initialization starting...');
        ...
        // 1. CHECK WHAT'S ACTUALLY IN THE MODAL
        const modalContent = modal.innerHTML;
        console.log('📋 Modal content preview:', modalContent.substring(0, 500));
        
        // 2. LOOK FOR EXISTING RADIO BUTTONS OR CREATE THEM
        let radios = modal.querySelectorAll('input[name="groupType"]');
        
        if (radios.length === 0) {
            console.log('🔨 No radio buttons found, creating them...');
            radios = createGroupTypeRadios(modal);
        }
        
        // 3. LOOK FOR EXISTING SECTIONS OR CREATE THEM
        let testimonialSection = modal.querySelector('#testimonialGroupSection, [data-section="testimonial"]');
        let informationalSection = modal.querySelector('#informationalGroupSection, [data-section="informational"]');
        
        if (!testimonialSection || !informationalSection) {
            console.log('🔨 Sections not found, creating them...');
            createModalSections(modal);
            
            // Get references after creation
            testimonialSection = modal.querySelector('#testimonialGroupSection');
            informationalSection = modal.querySelector('#informationalGroupSection');
        }
        
        // 4. SET UP EVENT LISTENERS
        console.log(`📻 Setting up ${radios.length} radio button listeners...`);
        ...
        const freshRadios = modal.querySelectorAll('input[name="groupType"]');
        freshRadios.forEach(radio => {
            radio.addEventListener('change', (e) => {
                console.log(`📻 Radio changed to: ${e.target.value}`);
                fixedUpdateGroupType(e.target.value);
            });
        });
        
        // 5. SET INITIAL STATE
        const initialType = modal.querySelector('input[name="groupType"]:checked')?.value || 'testimonial';
        console.log(`🎬 Initial group type: ${initialType}`);
        fixedUpdateGroupType(initialType);
        
        // 6. SET UP FORM SUBMISSION
        const form = modal.querySelector('form');
        if (form && typeof handleAddTestimonialGroup === 'function') {
            form.addEventListener('submit', handleAddTestimonialGroup);
    ...
    function createGroupTypeRadios(modal) {
        console.log('🎛️ Creating group type radio buttons...');
        
        // Find where to insert the radios (look for form or modal body)
        ...
        radioContainer.className = 'group-type-selection';
        ...
        radioContainer.innerHTML = `
            <h4 style="margin-top: 0; margin-bottom: 10px; color: #334155;">Select Group Type</h4>
            <div style="display: flex; gap: 20px; flex-wrap: wrap;">
                <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                    <input type="radio" name="groupType" value="testimonial" checked style="margin: 0;">
                ...
                <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                    <input type="radio" name="groupType" value="informational" style="margin: 0;">
                    <div style="display: flex; align-items: center; gap: 5px;">
                        <span style="font-size: 1.2em;">📚</span>
            ...
            <p style="margin-top: 10px; margin-bottom: 0; font-size: 0.9em; color: #64748b;">
                Testimonials show customer success stories. Informational videos are educational content.
            </p>
        `;
        
        // Insert at the beginning of the modal body
        if (modalBody.firstChild) {
            modalBody.insertBefore(radioContainer, modalBody.firstChild);
        } else {
            modalBody.appendChild(radioContainer);
        }
        
        return modalBody.querySelectorAll('input[name="groupType"]');
    }
    
    function createModalSections(modal) {
        console.log('📋 Creating modal sections...');
        ...
        testimonialSection.id = 'testimonialGroupSection';
        testimonialSection.className = 'group-section';
        testimonialSection.style.cssText = `
            margin: 15px 0;
            padding: 20px;
        ...
        testimonialSection.innerHTML = `
            <h4 style="margin-top: 0; color: #1e40af;">🎬 Testimonial Group</h4>
            <p style="color: #475569;">Create a group for customer testimonial videos that show real results and social proof.</p>
        `;
        
        // Create informational section  
        const informationalSection = document.createElement('div');
        informationalSection.id = 'informationalGroupSection';
        informationalSection.className = 'group-section';
        informationalSection.style.cssText = `
            margin: 15px 0;
            padding: 20px;
            ...
            display: none;
        `;
        informationalSection.innerHTML = `
            <h4 style="margin-top: 0; color: #047857;">📚 Informational Group</h4>
            <p style="color: #475569;">Create a group for educational or informational videos that explain features or processes.</p>
        `;
        
        // Insert after radio buttons
        const radioContainer = modal.querySelector('.group-type-selection');
        if (radioContainer) {
            modalBody.insertBefore(testimonialSection, radioContainer.nextSibling);
            modalBody.insertBefore(informationalSection, testimonialSection.nextSibling);
        } else {
            modalBody.appendChild(testimonialSection);
            modalBody.appendChild(informationalSection);
        }
        
        // Create concerns grid if needed
        if (!modal.querySelector('#groupConcernsGrid')) {
            const concernsGrid = document.createElement('div');
            concernsGrid.id = 'groupConcernsGrid';
            concernsGrid.className = 'concerns-grid';
            concernsGrid.style.cssText = `
                margin-top: 20px;
                padding: 15px;
                background: #f1f5f9;
                border-radius: 6px;
            ...
            concernsGrid.innerHTML = `
                <h5 style="margin-top: 0; margin-bottom: 10px;">Select Applicable Concerns</h5>
                <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(140px, 1fr)); gap: 8px;">
                    <!-- Concerns will be populated by updateGroupConcernsCheckboxes -->
                </div>
            `;
            
            // Insert after sections
            if (informationalSection.nextSibling) {
                modalBody.insertBefore(concernsGrid, informationalSection.nextSibling);
            } else {
                modalBody.appendChild(concernsGrid);
            }
        }
    }
    
    function fixedUpdateGroupType(type) {
        console.log(`🔄 Fixed updateGroupType: ${type}`);
        
        // Get sections
        const testimonialSection = document.getElementById('testimonialGroupSection');
        const informationalSection = document.getElementById('informationalGroupSection');
        
        if (!testimonialSection || !informationalSection) {
            console.error('❌ Sections not found for update');
            return;
        }
        ...
        if (type === 'testimonial') {
            testimonialSection.style.display = 'block';
            informationalSection.style.display = 'none';
            console.log('📊 Showing testimonial section');
        } else {
            testimonialSection.style.display = 'none';
            informationalSection.style.display = 'block';
            console.log('📚 Showing informational section');
        }
        
        // Update concerns checkboxes
        if (typeof updateGroupConcernsCheckboxes === 'function') {
            try {
                console.log('🎯 Calling updateGroupConcernsCheckboxes...');
                updateGroupConcernsCheckboxes(type);
            } catch (error) {
                console.warn('⚠️ Could not update concerns:', error.message);
            }
        } else {
            console.warn('⚠️ updateGroupConcernsCheckboxes function not found');
        }
    }
    
    // OVERRIDE THE updateGroupType FUNCTION TOO
    window.updateGroupType = fixedUpdateGroupType;
    
    // MONITOR AND AUTO-FIX
    console.log('👁️ Setting up modal monitor...');
    
    // Watch for modal show events
    const originalShowModal = window.showAddTestimonialGroupModal;
    if (originalShowModal) {
        window.showAddTestimonialGroupModal = function() {
            console.log('🚀 Modal show function called, ensuring fix is applied...');
            
            // Call original
            originalShowModal();
            
            // Run our fix after a short delay
            setTimeout(() => {
                console.log('🔧 Running modal fix after show...');
                fixedInitGroupCreationModal();
            }, 100);
        };
    }
    
    console.log('✅ Direct modal element fix loaded and ready!');
